      ...
      >
        <div className="p-4 border-b border-slate-200 dark:border-[#1e293b] flex justify-between items-center bg-slate-50/50 dark:bg-[#111c44]/50">
          <div className="relative w-full max-w-md">
            ...
            <input type="text" placeholder="Search referrals..." className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-xl outline-none focus:border-pink-500 transition-colors" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-[#1e293b]">
              {loading ? (
                ...
                <motion.tr 
                  ...
                  key={ref.id} className="hover:bg-slate-50 dark:hover:bg-[#152353] transition-colors"
                >
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{new Date(ref.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-100">{ref.patient?.name}</div>
                    <div className="text-xs text-slate-500">{ref.patient?.patientId}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-semibold text-slate-700 dark:text-slate-300">Dr. {ref.doctor?.name}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400 max-w-xs truncate" title={ref.reason}>{ref.reason}</td>
                  <td className="px-6 py-4">
            ...
            >
              ...
              <form onSubmit={handleCreateReferral} className="p-6 space-y-4">
                <div>
                  ...
                  <select required value={formData.patientId} onChange={e => setFormData({...formData, patientId: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-pink-500 transition-colors">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Referred To (External Specialist / Facility)</label>
                  <input required value={formData.referredTo} onChange={e => setFormData({...formData, referredTo: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-pink-500 transition-colors" placeholder="e.g. City Orthopedic Center" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Reason for Referral / Medical Notes</label>
                  <textarea required value={formData.reason} onChange={e => setFormData({...formData, reason: e.target.value})} rows={4} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-pink-500 transition-colors" placeholder="e.g. Patient requires advanced MRI scan for suspected..." />
                </div>
