      ...
      >
        <div className="p-4 border-b border-slate-200 dark:border-[#1e293b] flex justify-between items-center bg-slate-50/50 dark:bg-[#111c44]/50">
          <div className="relative w-full max-w-md">
            ...
            <input type="text" placeholder="Search lab tests..." className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-xl outline-none focus:border-purple-500 transition-colors" />
          </div>
        </div>
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      ...
                      className="text-purple-600 dark:text-purple-400 hover:text-purple-700 font-medium text-sm flex items-center gap-1 justify-end w-full"
                    >
            ...
            >
              ...
              <form onSubmit={handleOrder} className="p-6 space-y-4">
                <div>
                  ...
                  <select required value={orderForm.patientId} onChange={e => setOrderForm({...orderForm, patientId: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 transition-colors">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Test Name / Panel</label>
                  <input required value={orderForm.testName} onChange={e => setOrderForm({...orderForm, testName: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 transition-colors" placeholder="e.g. Complete Blood Count (CBC)" />
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleUpdateResult} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Diagnostic Report / Values</label>
                  <textarea required value={resultText} onChange={e => setResultText(e.target.value)} rows={6} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-purple-500 transition-colors" placeholder="Enter findings, numeric values, and pathological interpretation..." />
                </div>
                <div className="pt-4 flex justify-end gap-3">
