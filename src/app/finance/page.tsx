    ...
    <div className="p-4 lg:p-8 space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 }} className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-emerald-600 dark:text-emerald-400 font-medium text-sm">Total Revenue (Paid)</p>
            <h2 className="text-3xl font-bold text-slate-100">${data.totals.revenue.toLocaleString()}</h2>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="bg-amber-500/10 border border-amber-500/20 rounded-3xl p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-amber-600 dark:text-amber-400 font-medium text-sm">Accounts Receivable</p>
            <h2 className="text-3xl font-bold text-slate-100">${data.totals.pending.toLocaleString()}</h2>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="bg-red-500/10 border border-red-500/20 rounded-3xl p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-red-600 dark:text-red-400 font-medium text-sm">Overdue Collections</p>
            <h2 className="text-3xl font-bold text-slate-100">${data.totals.overdue.toLocaleString()}</h2>
          </div>
        </motion.div>
      </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-[#1e293b]">
              {loading ? (
                ...
                <motion.tr 
                  ...
                  key={inv.id} className="hover:bg-slate-50 dark:hover:bg-[#152353] transition-colors"
                >
                  <td className="px-6 py-4 font-mono text-sm text-slate-500">{inv.id.slice(-8).toUpperCase()}</td>
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-100">{inv.patient?.name}</div>
                    <div className="text-xs text-slate-500">{inv.patient?.patientId}</div>
                  </td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{new Date(inv.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{new Date(inv.dueDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 font-bold text-slate-100">${inv.totalAmount.toLocaleString()}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {inv.status !== 'paid' && (
                      <button 
                        ...
                        className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-sm font-semibold transition-colors flex items-center gap-1"
                      >
