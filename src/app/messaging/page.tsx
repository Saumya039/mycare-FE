...
export default function MessagingPage() {
 ...
 className="lg:col-span-1 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col overflow-hidden"
 >
 <div className="p-4 border-b border-slate-200 flex gap-2">
 <button 
 onClick={() => { setActiveTab("inbox"); setActiveMessage(null); }}
 className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors ${activeTab === "inbox" ? 'bg-blue-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100 '}`}
 >
 Inbox
 </button>
 <button 
 onClick={() => { setActiveTab("sent"); setActiveMessage(null); }}
 className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-colors ${activeTab === "sent" ? 'bg-blue-500 text-white shadow-md' : 'text-slate-500 hover:bg-slate-100 '}`}
 >
 Sent
 </button>
 </div>
 ...
 {loading ? (
 <p className="text-center text-slate-500 py-10">Loading messages...</p>
 ) : displayList.length === 0 ? (
 <p className="text-center text-slate-500 py-10">Empty {activeTab}</p>
 ) : displayList.map((msg: any) => (
 <button 
 key={msg.id}
 onClick={() => markAsRead(msg)}
 className={`w-full text-left p-4 rounded-2xl transition-all border ${activeMessage?.id === msg.id ? 'bg-blue-50 border-blue-200 shadow-sm' : 'bg-transparent border-transparent hover:bg-slate-50 '}`}
 >
 <div className="flex justify-between items-start mb-1">
 <span className={`font-bold ${!msg.isRead && activeTab === "inbox" ? 'text-slate-900 dark:text-white' : 'text-slate-700'}`}>
 ...
 <p className={`text-sm truncate ${!msg.isRead && activeTab === "inbox" ? 'text-slate-800 font-medium' : 'text-slate-500'}`}>
 {msg.content}
 </p>
 </button>
 ))}
 ...
 className="lg:col-span-2 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col overflow-hidden"
 >
 {activeMessage ? (
 ...
 <h2 className="text-2xl font-semibold text-slate-600 ">No Message Selected</h2>
 ...
 <div>
 <label className="block text-sm font-medium text-slate-600 mb-1">To: Staff Member</label>
 <select required value={formData.receiverId} onChange={e => setFormData({...formData, receiverId: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 transition-colors">
 ...
 <textarea required value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} rows={6} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-blue-500 transition-colors" placeholder="Type your message here..." />
 </div>
 
 <div className="pt-4 flex justify-end gap-3">
 <button type="button" onClick={() => setShowCompose(false)} className="px-6 py-3 rounded-xl font-medium text-slate-700 hover:bg-slate-100 transition-colors">
 Cancel
 </button>
