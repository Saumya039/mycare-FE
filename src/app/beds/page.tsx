  ...
  return (
    <div className="p-8 h-full bg-slate-950 text-slate-100 overflow-y-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-400">Live overview of hospital capacity and patient locations</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {depts.map(dept => {
            ...
            return (
              <div key={dept} className="bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-2xl overflow-hidden p-6">
                <h2 className="text-xl font-semibold mb-6 flex items-center gap-2 text-cyan-400 border-b border-slate-800 pb-4">
                  {dept} Ward
                  <span className="text-xs font-normal bg-slate-800 text-slate-300 px-2 py-1 rounded-full">
                  ...
                  {deptBeds.map(bed => (
                    <div 
                      ...
                      className={`relative rounded-xl p-4 border transition-all ${
                        bed.status === 'available' ? 'bg-slate-950 border-emerald-500/20 hover:border-emerald-500/50' :
                        'bg-slate-800/80 border-slate-700'
                      }`}
                    >
                      <div className="flex justify-between items-start mb-3">
                        <span className="text-xs font-mono font-bold text-slate-300">{bed.label}</span>
                        ...
                        <div>
                          ...
                          {(["ADMIN", "SUPER_ADMIN"].includes(session.user.role) || ["NURSE", "SUPER_ADMIN"].includes(session.user.role)) && (
                            <button 
                              ...
                              className="mt-3 w-full bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs py-1.5 rounded-lg flex items-center justify-center gap-1 transition-colors"
                            >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              ...
              <button onClick={() => setIsAssignModalOpen(false)} className="text-slate-400 hover:text-slate-200 transition-colors">
                ✕
              </button>
            </div>
            <form onSubmit={handleAssign} className="p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-slate-800 flex gap-3">
                <button type="button" onClick={() => setIsAssignModalOpen(false)} className="flex-1 px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm font-medium transition-colors">
                  Cancel
                </button>
