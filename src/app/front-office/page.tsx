      ...
      >
        <div className="p-4 border-b border-slate-200 dark:border-[#1e293b] flex justify-between items-center bg-slate-50/50 dark:bg-[#111c44]/50">
          <div className="relative w-full max-w-md">
            ...
            <input type="text" placeholder="Search visitors..." className="w-full pl-10 pr-4 py-2 bg-slate-900 border border-slate-800 rounded-xl outline-none focus:border-emerald-500 transition-colors" />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-[#1e293b]">
              {loading ? (
                ...
                <motion.tr 
                  ...
                  key={visitor.id} className="hover:bg-slate-50 dark:hover:bg-[#152353] transition-colors"
                >
                  <td className="px-6 py-4 text-slate-800 dark:text-slate-200 font-medium">{visitor.visitorName}</td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{visitor.phone}</td>
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{visitor.purpose}</td>
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-400 font-mono text-sm flex items-center gap-2">
                  ...
                  <td className="px-6 py-4">
                    {visitor.status === 'Active' ? (
                      ...
                      <span className="px-4 py-2 bg-slate-800 text-slate-500 rounded-lg text-sm font-medium flex items-center gap-2 w-max">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Visitor Name</label>
                  <input required value={formData.visitorName} onChange={e => setFormData({...formData, visitorName: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 transition-colors" placeholder="John Doe" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Phone Number</label>
                  <input required value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 transition-colors" placeholder="+1 (555) 000-0000" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Whom To Meet</label>
                  <input required value={formData.whomToMeet} onChange={e => setFormData({...formData, whomToMeet: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 transition-colors" placeholder="Dr. Smith (Cardiology) or Room 204" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Purpose of Visit</label>
                  <input required value={formData.purpose} onChange={e => setFormData({...formData, purpose: e.target.value})} type="text" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-emerald-500 transition-colors" placeholder="Patient meeting, Vendor, Interview..." />
                </div>
