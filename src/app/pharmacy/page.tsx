...
export default function PharmacyPage() {
 ...
 className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col h-[600px]"
 >
 ...
 <div key={rx.id} className="p-4 rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
 <div>
 <div className="flex items-center gap-2 mb-1">
 <span className="font-bold text-slate-900">{rx.medicationName}</span>
 <span className="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 ">{rx.dosage}</span>
 </div>
 <div className="text-sm text-slate-600 ">
 Patient: <span className="font-medium text-slate-800 ">{rx.patient?.name}</span> ({rx.patient?.patientId})
 </div>
 ...
 <thead className="bg-slate-50/50 text-slate-500 text-sm sticky top-0 ">
 <tr>
 ...
 <button onClick={() => openRestock(item)} className="text-emerald-600 hover:text-emerald-700 font-medium text-sm underline decoration-emerald-500/30 underline-offset-4">
 Restock
 </button>
 </td>
 </tr>
 ...
 className="relative w-full max-w-sm bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden"
 >
 ...
 <input disabled value={selectedItem.itemName} type="text" className="w-full px-4 py-3 bg-slate-50 text-slate-500 border border-slate-200 rounded-xl outline-none" />
 </div>
 <div>
 <label className="block text-sm font-medium text-slate-600 mb-1">Units to Add</label>
 <input required value={restockQty} onChange={e => setRestockQty(e.target.value)} type="number" min="1" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-emerald-500 transition-colors" placeholder="e.g. 50" />
 </div>
 
 <div className="pt-4 flex justify-end gap-3">
 <button type="button" onClick={() => setShowRestockModal(false)} className="px-5 py-2.5 rounded-xl font-medium text-slate-700 hover:bg-slate-100 transition-colors">
 Cancel
 </button>
