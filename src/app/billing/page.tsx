              ...
              {loading ? (
                ...
                invoices.map(inv => (
                  <tr key={inv.id} className="hover:bg-slate-800/20 transition-colors">
                    <td className="p-4 font-mono text-sm text-slate-400">{inv.id.substring(0, 8).toUpperCase()}</td>
                    <td className="p-4">
                      <p className="font-medium text-slate-200">{inv.patient?.name}</p>
                    ...
                    <td className="p-4 text-sm text-slate-400 capitalize">{inv.insuranceClaimStatus}</td>
                    <td className="p-4 text-sm text-slate-300">{new Date(inv.dueDate).toLocaleDateString()}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Select Patient</label>
                <select 
                  required 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Insurance Claim Status</label>
                <select 
