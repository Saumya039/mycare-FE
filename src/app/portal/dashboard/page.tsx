...
export default function PortalDashboard() {
 ...
 if (loading) {
 return <div className="min-h-screen bg-slate-50 flex items-center justify-center text-cyan-400">Loading your records...</div>
 }
 ...
 {/* Header */}
 <header className="border-b border-slate-200 bg-white p-6 flex justify-between items-center">
 <div className="flex items-center gap-3">
 <Shield className="w-8 h-8 text-cyan-500" />
 <div>
 <h1 className="text-xl font-bold text-white">Patient Portal</h1>
 <p className="text-xs text-slate-500">Sevra Technologies Serva AI</p>
 </div>
 </div>
 <button onClick={handleLogout} className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors text-sm font-medium">
 ...
 <h3 className="font-semibold text-slate-500 uppercase tracking-wider text-xs mb-4 flex items-center gap-2">
 ...
 <p className="text-sm text-slate-500">Not set yet</p>
 )}
 </div>
 </div>
 ...
 <p className="font-medium text-slate-800">{data.diagnosis || "Pending Evaluation"}</p>
 </div>
 <div>
 ...
 <p className="font-medium text-slate-800">{data.departmentName}</p>
 </div>
 <div>
 ...
 <div key={px.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl">
 <p className="font-bold text-slate-800">{px.medicationName} <span className="text-slate-500 font-normal ml-1">({px.dosage})</span></p>
 <p className="text-sm text-slate-500 mt-1">{px.frequency} for {px.duration}</p>
 </div>
 ))}
 </div>
 )}
 ...
 <p className="font-bold text-slate-800">{new Date(apt.date).toLocaleDateString()}</p>
 <p className="text-sm text-slate-500 mt-1">{apt.reason}</p>
 </div>
 ))}
 </div>
 )}
 ...
 <div key={lab.id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl">
