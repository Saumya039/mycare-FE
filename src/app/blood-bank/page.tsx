...
export default function BloodBankPage() {
 ...
 <p className="text-slate-500">Manage blood inventory and transfusions</p>
 </div>
 ...
 {bloodTypes.map(type => (
 <div key={type} className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center relative overflow-hidden group">
 <div className="absolute inset-0 bg-red-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
 <h3 className="text-2xl font-bold text-slate-800 mb-1 z-10">{type}</h3>
 ...
 className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-red-500"
 />
 </div>
 </div>
 ...
 <td className="p-4 font-mono text-sm text-slate-500">{bag.id.substring(0, 8).toUpperCase()}</td>
 <td className="p-4 font-bold text-red-400 text-lg">{bag.bloodType}</td>
 <td className="p-4 text-sm text-slate-700">{new Date(bag.collectionDate).toLocaleDateString()}</td>
 <td className="p-4 text-sm text-slate-700">{new Date(bag.expiryDate).toLocaleDateString()}</td>
 <td className="p-4">
 ...
 <div>
 <label className="block text-sm font-medium text-slate-500 mb-1">Blood Type</label>
 <select 
 required 
 value={form.bloodType} 
 onChange={(e) => setForm({...form, bloodType: e.target.value})}
 className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-slate-800 focus:outline-none focus:border-red-500"
 >
 ...
 <div>
 <label className="block text-sm font-medium text-slate-500 mb-1">Number of Units (Bags)</label>
 <input 
